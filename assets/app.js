    ...
    function renderFilmPipeline() {
      ...
      FilmEngine.Core.render({ ctx, canvas, previewCanvas, state, filmDatabase, applyLensDistortion, applyChroma });
    ...
    function installMaskControls() {
      document.querySelectorAll('.mask-slot').forEach(slot => {
        ...
        angleInput.addEventListener('input', () => { cfg.custom.angle = parseFloat(angleInput.value); syncLabels(); requestRender(); });
        cxInput.addEventListener('input', () => { cfg.custom.cx = parseFloat(cxInput.value); syncLabels(); requestRender(); });
        cyInput.addEventListener('input', () => { cfg.custom.cy = parseFloat(cyInput.value); syncLabels(); requestRender(); });
        radiusInput.addEventListener('input', () => { cfg.custom.radius = parseFloat(radiusInput.value); syncLabels(); requestRender(); });
        featherInput.addEventListener('input', () => { cfg.custom.feather = parseFloat(featherInput.value); syncLabels(); requestRender(); });
