    ...
    function preparePreviewBuffer(maxDim = 900) {
      ...
      // исходного изображения, задаётся интерактивным окном кадрирования.
      // null/undefined = используется кадр целиком.
      const r = state.cropRect;
      if (r && r.w > 0.01 && r.h > 0.01) {
        sx = Math.round(r.x * sourceImg.width);
        sy = Math.round(r.y * sourceImg.height);
        sw = Math.round(r.w * sourceImg.width);
        sh = Math.round(r.h * sourceImg.height);
      }
    ...
    document.getElementById('btnOpenCrop').addEventListener('click', () => {
      if (!sourceImg) return;
      SvemaCrop.open(sourceImg, state.cropRect, (rect) => {
