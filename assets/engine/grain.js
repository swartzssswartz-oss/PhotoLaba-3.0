    ...
    (function (root) {
      ...
      // 2D-хэш ячейки зерна (не по плоскому индексу пикселя!) — раньше ячейка
      // считалась как floor(px / size), где px = y*w+x: при переходе через
      // край строки такая «ячейка» рвётся и уезжает по диагонали (ширина
      // кадра почти никогда не кратна размеру зерна), из-за чего крупное
      // зерно на глаз читалось не органичными крапинками, а едва заметной
      // диагональной штриховкой. Комбинируем x/y отдельно через 2 больших
      // простых числа — классический дешёвый 2D-хэш без такой асимметрии.
      function cellHash2D(cx, cy) {
      ...
      function apply(data, w, h, state, grainMultFromDeveloper, pushGrainBonus) {
        ...
        for (let i = 0; i < data.length; i += 4) {
          ...
          const cxFine = Math.floor(x / sizeScaleFine), cyFine = Math.floor(y / sizeScaleFine);
          const cxCoarse = Math.floor(x / sizeScaleCoarse), cyCoarse = Math.floor(y / sizeScaleCoarse);
          const hFine = cellHash2D(cxFine, cyFine);
          const hCoarse = cellHash2D(cxCoarse, cyCoarse);

          const nFine = hash01((hFine + seed) | 0) - 0.5;
          const nCoarse = hash01((hCoarse + seed * 7 + 91) | 0) - 0.5;
          ...
          if (colorAmt <= 0.001) {
            ...
            const nR = hash01((hFine + seed + 15485863) | 0) - 0.5;
            const nG = hash01((hFine + seed + 104729) | 0) - 0.5;
            const nB = hash01((hFine + seed + 217645) | 0) - 0.5;
