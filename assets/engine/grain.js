    ...
    (function (root) {
      ...
      function apply(data, w, h, state, grainMultFromDeveloper, pushGrainBonus) {
        const baseAmount = (state.grain + (pushGrainBonus || 0)) * 0.6 * (grainMultFromDeveloper || 1);
        if (baseAmount <= 0) return;

        const sizeScaleFine = Math.max(0.4, state.grainSize / 10);
        ...
        for (let i = 0; i < data.length; i += 4) {
          ...
          if (colorAmt <= 0.001) {
            ...
            const noiseR = (n * (1 - colorAmt) + n * colorAmt) * amount;
