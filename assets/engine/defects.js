    ...
    (function (root) {
      ...
      function render(ctx, w, h, state, refImageData, age, unevenness) {
        ...
        // у реальной неравномерной циркуляции химии. unevenness (из FilmEngine.Developer,
        // растёт при низкой ажитации) усиливает уже выставленный вручную bromide — не
        // рисует потёки из ничего при bromide=0, а делает эффект от низкой ажитации
        // физически заметным именно там, где он и должен быть: на потёках химии.
        if (state.bromide > 0) {
          const baseAlpha = (state.bromide / 240) * (1 + (unevenness || 0) * 0.7);
