    ...
    //  3b. Toe/Shoulder — асимметричная широта по типу плёнки (негатив/слайд)
    ...
    //  5b. Acutance / линии Мэки — микро-контраст границ от химии проявки (dev.acutance)
    //  6. Grain Engine
    ...
    (function (root) {
      ...
      function render(opts) {
        ...
        // ---- Асимметрия toe/shoulder по kind: обычный contrastFactor ниже — симметричный
        // поворот вокруг 128, одинаковый для обоих концов. У реальной плёнки это не так:
        // у негатива огромная широта по светам (фирменная черта — можно передержать на
        // несколько стоп и не потерять свет) и мягкий переход в тенях; у слайда — резкий
        // обрыв на обоих концах (узкая широта). Правим только крайние ~40 значений с
        // каждого конца (per-channel, как и сам contrastFactor) — середину и уже
        // подобранные per-stock recipe-контрасты это не трогает.
        const TOE_SHOULDER = { negative: { toe: 8, shoulder: 14 }, slide: { toe: -5, shoulder: -10 } };
        const tsShape = TOE_SHOULDER[chemKind] || TOE_SHOULDER.negative;
        const toeAmt = tsShape.toe, shoulderAmt = tsShape.shoulder;

        // ---- Оранжевая маска цветного негатива: вуаль/база (fogDensity ниже) у
        // настоящего цветного негатива не нейтрально-серая, а тёплая — остаток
        // корректирующих цветных масок в слоях эмульсии. Отсюда характерный тёплый
        // оттенок теней у сканов негатива, которого нет у слайда/ч/б (там база
        // действительно нейтральна, туда эту тонировку добавлять физически неверно).
        const isColorNegOrangeMask = filmKind === 'negative' && !isBWStock && !isIRStock;
        ...
        for (let px = 0; px < w * h; px++) {
          ...
          // ---- 3b. Toe/Shoulder асимметрия (см. TOE_SHOULDER выше) -------------
          if (r < 40) { const t = r < 0 ? 1 : (1 - r / 40); r += toeAmt * t; }
          else if (r > 215) { const t = r > 255 ? 1 : (r - 215) / 40; r -= shoulderAmt * t; }
          if (g < 40) { const t = g < 0 ? 1 : (1 - g / 40); g += toeAmt * t; }
          else if (g > 215) { const t = g > 255 ? 1 : (g - 215) / 40; g -= shoulderAmt * t; }
          if (b < 40) { const t = b < 0 ? 1 : (1 - b / 40); b += toeAmt * t; }
          else if (b > 215) { const t = b > 255 ? 1 : (b - 215) / 40; b -= shoulderAmt * t; }
          ...
          if (fogDensity > 0) {
            if (isColorNegOrangeMask) {
              // См. isColorNegOrangeMask выше: вуаль тонируется тёплым (оранжевым),
              // а не нейтральным серым — та самая тёплая тень цветного негатива.
              const fogR = fogDensity * 1.35, fogG = fogDensity * 0.85, fogB = fogDensity * 0.45;
              r = r * (1 - fogDensity / 255) + fogR;
              g = g * (1 - fogDensity / 255) + fogG;
              b = b * (1 - fogDensity / 255) + fogB;
            } else {
        ...
        // ---- Эффект прилегания / линии Мэки (unsharp небольшого радиуса,
        // сила из химии проявки — dev.acutance) — ДО зерна, чтобы не
        // резать синтетический шум зерна, а не только реальные границы.
        FilmEngine.Acutance.render(w, h, imgData, dev.acutance, 1.6);
        ...
        FilmEngine.Defects.render(ctx, w, h, state, imgData, age, dev.unevenness);
